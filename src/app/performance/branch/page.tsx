  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 pb-20">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <Card className="shadow-lg border-slate-200 border-l-4 border-l-orange-500 bg-white overflow-hidden rounded-2xl"><CardHeader className="pb-2 text-[10px] font-black uppercase text-orange-600 bg-slate-50/50">Methodology Gaps</CardHeader><CardContent className="pt-4 flex items-end justify-between"><span className="text-5xl font-black text-orange-600 tracking-tighter">{aggregateStats.amended}</span><div className="p-2 bg-orange-50 rounded-lg"><ShieldAlert className="w-4 h-4 text-orange-600" /></div></CardContent></Card>
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        {branchMetrics.map((branch) => {
          ...
          return (
            <Card key={branch.name} className="shadow-xl border-slate-200 overflow-hidden group hover:border-primary/40 transition-all rounded-3xl bg-white"><CardHeader className="bg-slate-50/80 border-b p-6 flex flex-row items-start justify-between"><div><CardTitle className="text-xl font-black text-slate-900">{branch.name}</CardTitle><p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1 flex items-center gap-1.5"><Building2 className="w-3 h-3" /> {branch.district} Regional Branch</p></div><Badge variant="outline" className="bg-white text-primary border-primary/20 font-black h-7">{branch.volume} Cases</Badge></CardHeader><CardContent className="pt-8 px-6 space-y-8"><div className="grid grid-cols-2 gap-6"><div className="space-y-1"><p className="text-[10px] font-bold text-slate-400 uppercase tracking-tight">Successfully Authorized</p><div className="flex items-center gap-2"><span className="text-3xl font-black text-emerald-600">{branch.approved}</span><ShieldCheck className="w-4 h-4 text-emerald-600/30" /></div></div><div className="space-y-1"><p className="text-[10px] font-bold text-slate-400 uppercase tracking-tight">Methodology Gaps</p><div className="flex items-center gap-2"><span className="text-3xl font-black text-orange-600">{branch.amended}</span><ShieldAlert className="w-4 h-4 text-orange-600/30" /></div></div></div><div className="space-y-3"><div className="flex justify-between items-end"><div className="space-y-0.5"><p className="text-[10px] font-black text-slate-400 uppercase">Accuracy Index</p><p className={cn("text-lg font-black leading-none", efficiency >= 90 ? "text-emerald-600" : efficiency >= 70 ? "text-primary" : "text-orange-600")}>{efficiency}%</p></div><ArrowUpRight className="w-4 h-4 text-slate-200" /></div><Progress value={efficiency} className={cn("h-2 bg-slate-100", efficiency >= 90 ? "[&>div]:bg-emerald-500" : efficiency >= 70 ? "[&>div]:bg-primary" : "[&>div]:bg-orange-500")} /></div></CardContent></Card>
          );
        })}
      </div>
    </div>
