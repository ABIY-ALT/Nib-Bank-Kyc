      ...
      <Card className="border-slate-200 shadow-sm">
        ...
        <CardContent className="pt-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="space-y-2">
              <label className="text-xs font-bold text-slate-500 uppercase tracking-widest flex items-center gap-2">
                <CalendarIcon className="w-3 h-3" /> Audit Horizon
              </label>
              <Button variant="outline" className="w-full h-11 justify-start text-left font-normal border-slate-200 bg-slate-50/50" disabled>
                Last 30 Days (Regulatory Standard)
              </Button>
            </div>
          </div>
        ...
        <Card className="border-slate-200 shadow-xl animate-in slide-in-from-top-4 duration-500 overflow-hidden">
          <CardHeader className="bg-slate-900 text-white rounded-t-lg p-6">
            <div className="flex justify-between items-center">
              <div>
                <CardTitle className="text-2xl font-bold tracking-tight">Audit Archive: {selectedDistrict} / {selectedBranch}</CardTitle>
                <p className="text-slate-400 text-sm mt-1 font-medium">Verified for regulatory standards on {new Date().toLocaleDateString()}</p>
              </div>
