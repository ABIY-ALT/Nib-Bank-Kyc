...
export default function BranchReportsPage() {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-300 pb-20">
      ...
      {reportData && (
        <Card className="border-slate-200 shadow-xl overflow-hidden rounded-[2rem] bg-white"><CardHeader className="bg-slate-900 text-white p-8"><div className="flex justify-between items-center"><div><CardTitle className="text-2xl font-bold tracking-tight">Audit Archive: {selectedDistrict === 'all' ? 'Global' : selectedDistrict}</CardTitle><p className="text-slate-400 text-sm mt-1 font-medium italic">Validated records aggregated on {format(new Date(), 'dd/MM/yyyy')}</p></div><Badge className="bg-primary/20 text-white border-primary/40 font-black px-4 h-8">{reportData.length} Records</Badge></div></CardHeader><CardContent className="p-0"><Table><TableHeader className="bg-slate-50/80"><TableRow><TableHead className="font-black py-4 pl-8 text-[11px] uppercase">Case ID</TableHead><TableHead className="font-black text-[11px] uppercase">Customer</TableHead><TableHead className="font-black text-[11px] uppercase">Node</TableHead><TableHead className="font-black text-center text-[11px] uppercase">Status</TableHead><TableHead className="font-black text-right pr-8 text-[11px] uppercase">Review Date</TableHead></TableRow></TableHeader><TableBody>{reportData.map((sub) => (<TableRow key={sub.id} className="hover:bg-slate-50 transition-colors border-b"><TableCell className="font-bold text-primary pl-8">{sub.id}</TableCell><TableCell className="font-bold text-slate-900">{sub.customerName}</TableCell><TableCell className="font-medium text-slate-600"><div className="flex flex-col"><span className="font-bold">{sub.branchName}</span><span className="text-[9px] uppercase font-black text-slate-400">{sub.districtName} District</span></div></TableCell><TableCell className="text-center"><Badge className="font-bold">{sub.status}</Badge></TableCell><TableCell className="text-xs text-slate-500 font-bold text-right pr-8">{sub.submittedAt ? format(new Date(sub.submittedAt), 'MMM dd, yyyy') : 'N/A'}</TableCell></TableRow>))}</TableBody></Table></CardContent></Card>
      )}
    </div>
  );
}
